...
#define LIST_SIZE_DEFAULT 25
#define TIME_OUT_DEFAULT 600
#define MAVLINK_RESEND_TIME_MS 30000 // only resend mavlink message for the same vehicle after this time if there has been no change

const AP_Param::GroupInfo AP_AIS::var_info[] = {

    // @Param: TYPE
    // @DisplayName: AIS recever type
    // @Description: AIS recever type
    ...
    // @Range: 1 100
    // @User: Advanced
    AP_GROUPINFO("LIST_MAX", 2, AP_AIS, _max_list, LIST_SIZE_DEFAULT),
    ...
    // @Range: 1 2000
    // @User: Advanced
    AP_GROUPINFO("TIME_OUT", 3, AP_AIS, _time_out, TIME_OUT_DEFAULT),

    // @Param: OPTIONS
    // @DisplayName: AIS options
    // @Description: Bitmask of AIS options
    // @Bitmask: 0:Log all AIVDM messages,1:Log only unsupported AIVDM messages,2:Log decoded messages
    // @User: Advanced
    AP_GROUPINFO("OPTIONS", 4, AP_AIS, _options, AIS_OPTIONS_LOG_UNSUPPORTED_RAW),
...
void AP_AIS::update()
{
    ...
    while (nbytes-- > 0) {
        char c = _uart->read();
        if (decode(c)) {
            const bool log_all = (_options & AIS_OPTIONS_LOG_ALL_RAW) != 0;
            const bool log_unsupported = ((_options & AIS_OPTIONS_LOG_UNSUPPORTED_RAW) != 0) && !log_all; // only log unsupported if not logging all
            ...
            if (_incoming.num == 1 && _incoming.total == 1) {
                ...
                uint8_t index = 0;
                uint8_t msg_parts[_incoming.total - 1];
                ...
                // combine packets
                char multi[65*_incoming.total];
                strncpy(multi,_AIVDM_buffer[msg_parts[0]].payload,65);
                for (uint8_t i = 1; i < _incoming.total - 1; i++) {
                    strcat(multi,_AIVDM_buffer[msg_parts[i]].payload);
                }
                strcat(multi,_incoming.payload);
...
void AP_AIS::send(mavlink_channel_t chan)
{
    ...
    while (search_length < list_size) {
        ...
        if (_list[_send_index].last_update_ms != 0 &&
            (_list[_send_index].last_send_ms < _list[_send_index].last_update_ms || now -_list[_send_index].last_send_ms > MAVLINK_RESEND_TIME_MS)) {
...
void AP_AIS::buffer_shift(uint8_t i)
{
    for (uint8_t n = i;  n < (AIVDM_BUFFER_SIZE - 1); n++) {
        ...
        strncpy(_AIVDM_buffer[n].payload,_AIVDM_buffer[n+1].payload,65);
    }
...
void AP_AIS::clear_list_item(uint16_t index)
{
    if (index < _list.max_items()) {
        _list[index].last_update_ms = 0;
        _list[index].info.MMSI = 0; // uint32_t Mobile Marine Service Identifier, 9 decimal digits
        _list[index].info.lat = 0; // int32_t [degE7] Latitude
        _list[index].info.lon = 0; // int32_t [degE7] Longitude
        _list[index].info.COG = 0; // uint16_t [cdeg] Course over ground
        _list[index].info.heading = 0; // uint16_t [cdeg] True heading
        _list[index].info.velocity = 0; // uint16_t [cm/s] Speed over ground
        _list[index].info.dimension_bow = 0; // uint16_t [m] Distance from lat/lon location to bow
        _list[index].info.dimension_stern = 0; // uint16_t [m] Distance from lat/lon location to stern
        _list[index].info.tslc = 0; // uint16_t [s] Time since last communication in seconds
        _list[index].info.flags = 0; // uint16_t Bitmask to indicate various statuses including valid data fields
        _list[index].info.turn_rate = 0; // int8_t [cdeg/s] Turn rate
        _list[index].info.navigational_status = 0; // uint8_t Navigational status
        _list[index].info.type = 0; // uint8_t Type of vessels
        _list[index].info.dimension_port = 0; // uint8_t [m] Distance from lat/lon location to port side
        _list[index].info.dimension_starboard = 0; // uint8_t [m] Distance from lat/lon location to starboard side
        _list[index].info.callsign[0] = 0; // char The vessel callsign
...
bool AP_AIS::payload_decode(const char *payload)
{
    ...
    uint8_t type = payload_char_decode(payload[0]);
...
bool AP_AIS::decode_position_report(const char *payload, uint8_t type)
{
    ...
    if ((_options & AIS_OPTIONS_LOG_DECODED) != 0) {
...
bool AP_AIS::decode_static_and_voyage_data(const char *payload)
{
    ...
    strncpy(_list[index].info.callsign,call_sign,7); // char The vessel callsign
    strncpy(_list[index].info.name, name,20); // char The vessel name
...
void AP_AIS::log_raw(const AIVDM *msg)
{
    ...
    strncpy(pkt.payload, msg->payload, sizeof(pkt.payload));
    AP::logger().WriteBlock(&pkt, sizeof(pkt));
}
